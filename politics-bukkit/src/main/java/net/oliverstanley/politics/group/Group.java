...
import net.oliverstanley.politics.universe.UniverseRules;
import net.oliverstanley.politics.util.collect.CollectorUtil;
import net.oliverstanley.politics.util.config.PropertyDeserializationException;
import net.oliverstanley.politics.util.config.PropertySerializer;
 ...
 * have child or parent Groups and may or may not have direct members. It may also own land in the form of {@link Plot}s.
 */
public final class Group implements Comparable<Group> {
    ...
    public Stream<Group> children() {
        return universe.childGroups(this);
    }

    public Stream<UUID> immediatePlayers() {
        return players.keySet().stream();
    }

    public Stream<Player> immediateOnlinePlayers() {
        return immediatePlayers().map(Politics.getServer()::getPlayer).filter(Objects::nonNull);
    }

    public Stream<UUID> players() {
        return Stream.concat(immediatePlayers(), children().flatMap(Group::immediatePlayers)).distinct();
    }

    public Stream<Privilege> memberPrivileges(UUID playerId) {
        return getRole(playerId).map(Role::privileges).orElseGet(Stream::empty);
    }
    ...
    public Optional<Group> getParent() {
        return universe.groups().filter(this::isParent).findAny();
    }
    ...
    public boolean hasChild(Group other) {
        return universe.childGroups(other).anyMatch(this::equals);
    }
    ...
    public boolean isMember(UUID player) {
        return isImmediateMember(player) || children().anyMatch(child -> child.isMember(player));
    }
